...
def get_1d_data(f1, config1, f2, config2, f3, config3):
    train_data1, val_data1, test_data1 = get_data(f1, config1)
    train_data2, val_data2, test_data2 = get_data(f2, config2)
    train_data3, val_data3, test_data3 = get_data(f3, config3)
...
def run_training(config, prefix, seed):
    ...
    f.write(
        f"batch_size={config['batch_size']}, learning_rate={config['learning_rate']}, weight_decay={config['weight_decay']}, dropout={config['dropout']}\n")
    f.write("TEST VALUE FROM LAST EPOCH: {0:5f}\n".format(test_value))
    ...
    f.write("TEST VALUE BEST LAST EPOCH: {0:5f}\n".format(test_value))
    f.close()
                ...
                for dropout in [0]:
                    for num_samples in [10, 100, 1000]:
                        ...
                        for seed in range(1):
                            ...
                            run_training(train_args, prefix, seed)
