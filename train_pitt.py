...
from train_pitt_1d_fusion import train_fusion
...
def get_data(f, config):
    ...
    train_loader = torch.utils.data.DataLoader(ConcatDataset([train_data, train_data, train_data]),
                                               batch_size=config['batch_size'],
                                               num_workers=config['num_workers'], shuffle=True,
                                               generator=torch.Generator(device='cuda'))
    val_loader = torch.utils.data.DataLoader(ConcatDataset([val_data, val_data, val_data]),
                                             batch_size=config['batch_size'],
                                             num_workers=config['num_workers'], shuffle=False,
                                             generator=torch.Generator(device='cuda'))
    test_loader = torch.utils.data.DataLoader(ConcatDataset([test_data, test_data, test_data]),
                                              batch_size=config['batch_size'],
                                              num_workers=config['num_workers'], shuffle=False,
                                              generator=torch.Generator(device='cuda'))
...
if __name__ == '__main__':
    ...
    for flnm, data_name, batch_size, lr, wd, do in [
        ...
        for num_samples in [
            ...
            train_args['dropout'] = do
            train_args['num_samples'] = num_samples

            prefix = train_args['flnm'] + "_" + train_args['data_name'].split("_")[0] + "_" + train_args[
                'train_style'] + "_" + \
                     train_args['embedding'] + "_" + str(num_samples)
            train_args['prefix'] = prefix
            os.makedirs(
                "{}{}_{}_{}".format(train_args['results_dir'], train_args['transformer'], train_args['neural_operator'],
                                    prefix),
                exist_ok=True)
