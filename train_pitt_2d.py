...
def get_neural_operator(model_name, config):
    if (model_name == "fno"):
        ...
        model = UNet2d(in_channels=config['initial_step'], init_features=config['init_features'],
        ...
        encoder = SpatialTemporalEncoder2D(input_channels=config['input_channels'], in_emb_dim=config['in_emb_dim'],
                                           out_seq_emb_dim=config['out_seq_emb_dim'], depth=config['depth'],
                                           heads=config['heads'])
        # , dropout=config['dropout'],
        # res=config['enc_res'])
...
def get_transformer(model_name, config):
    ...
    if (config['embedding'] == "standard"):
        ...
        transformer = StandardPhysicsInformedTokenTransformer2D(100, config['hidden'], config['layers'],
                                                                config['heads'],
                                                                output_dim1=config['num_x'],
                                                                output_dim2=config['num_y'], dropout=config['dropout'],
        ...
        transformer = PhysicsInformedTokenTransformer2D(100, config['hidden'], config['layers'], config['heads'],
                                                        output_dim1=config['num_x'], output_dim2=config['num_y'],
                                                        dropout=config['dropout'],
                                                        neural_operator=neural_operator).to(device=device)
    else:
...
def run_training(config, prefix):
    ...
    for epoch in tqdm(range(config['epochs'])):
        ...
        for bn, (x0, y, grid, tokens, t) in enumerate(train_loader):
            ...
            if (bn % 100 == 0 and len(train_loader) >= 1000):
                ...
                prod_mat = torch.mul(transformer.query_matrix.detach().cpu()[0],
                                     transformer.key_matrix.detach().cpu()[0])
                im2 = ax[0][2].imshow(prod_mat, vmin=prod_mat.mean() - prod_mat.std(),
                                      vmax=prod_mat.mean() + prod_mat.std(), cmap='bwr')
                ...
                im5 = ax[1][2].imshow(prod_mat2, vmin=prod_mat2.mean() - prod_mat2.std(),
                                      vmax=prod_mat2.mean() + prod_mat2.std(), cmap='bwr')
    ...
    np.save(
        "{}{}_{}_{}/test_vals_{}.npy".format(config['results_dir'], config['model'], config['neural_operator'], prefix,
                                             seed), test_vals)
...
if __name__ == '__main__':
    ...
    os.makedirs(
        "{}{}_{}_{}".format(train_args['results_dir'], train_args['model'], train_args['neural_operator'], prefix),
        exist_ok=True)
